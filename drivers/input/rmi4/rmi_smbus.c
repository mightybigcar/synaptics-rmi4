...
static int rmi_set_page(struct rmi_transport_device *xport, u8 page)
{
	...
	struct rmi_smb_data *data = xport->data;
...
static int rmi_smb_v1_write_block(struct rmi_transport_device *xport, u16 addr,
				  u8 *buf, int len)
{
	struct i2c_client *client = to_i2c_client(xport->dev);
	struct rmi_smb_data *data = xport->data;
	...
	if (RMI_SMB_PAGE(addr) != data->page) {
		retval = rmi_set_page(xport, RMI_SMB_PAGE(addr));
		if (retval < 0)
			goto exit;
	}
	...
	xport->info.tx_count++;
	xport->info.tx_bytes += sizeof(tx_size);
...
static int rmi_smb_v1_read_block(struct rmi_transport_device *xport, u16 addr,
			u8 *buf, int len)
{
	struct i2c_client *client = to_i2c_client(xport->dev);
	struct rmi_smb_data *data = xport->data;
...
static int smb_v2_block_write(struct rmi_transport_device *xport,
...
static int rmi_smb_v2_get_command_code(struct rmi_transport_device *xport,
		u16 rmiaddr, int bytecount, bool isread, u8 *commandcode)
{
	...
	retval = smb_v2_block_write(xport, i+0x80, (u8 *) mapping_data,
				    sizeof(mapping_data));
	if (retval < 0) {
...
static int rmi_smb_v2_write_block(struct rmi_transport_device *xport, u16 rmiaddr,
		u8 *databuff, int len)
{
	int retval = 0;
	u8 commandcode;
	...
	while (len > 0) {  /* while more than 32 bytes */
		...
		retval = rmi_smb_v2_get_command_code(xport, rmiaddr, block_len,
			false, &commandcode);
		if (retval < 0)
			goto exit;
		/* write to smb device */
		retval = smb_v2_block_write(xport, commandcode,
			databuff, block_len);
		if (retval < 0)
			goto exit;
...
static int smb_v2_block_read(struct rmi_transport_device *xport,
			u8 commandcode, u8 *buf, int len)
{
	struct i2c_client *client = to_i2c_client(xport->dev);
	struct rmi_smb_data *data = xport->data;
...
static int rmi_smb_v2_read_block(struct rmi_transport_device *xport, u16 rmiaddr,
					u8 *databuff, int len)
{
	struct rmi_smb_data *data = xport->data;
	int retval;
	...
	while (len > 0) {
		...
		if (retval < 0)
			goto exit;
		/* read to smb device */
		retval = smb_v2_block_read(xport, commandcode,
			databuff, block_len);
		if (retval < 0)
			goto exit;
				  ...
				  const struct i2c_device_id *id)
{
	...
	case 1:
		...
		* known state, and (b) make sure we can talk to the device. */
		...
		rmi_transport->write_block = rmi_smb_v1_write_block;
		rmi_transport->read_block = rmi_smb_v1_read_block;
		rmi_transport->info.proto = smb_v1_proto_name;
		break;
	case 2:
		/* SMBv2 */
		...
		rmi_transport->write_block	= rmi_smb_v2_write_block;
		rmi_transport->read_block	= rmi_smb_v2_read_block;
		rmi_transport->info.proto	= smb_v2_proto_name;
		break;
	default:
