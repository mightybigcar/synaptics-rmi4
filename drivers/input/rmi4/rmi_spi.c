...
struct rmi_spi_data {
	...
	int page;
	int (*set_page) (struct rmi_transport_device *xport, u8 page);
...
{
	struct rmi_transport_device *xport = p;
	struct rmi_spi_data *data = xport->data;
...
static int rmi_spi_xfer(struct rmi_transport_device *xport,
		    const u8 *txbuf, const unsigned n_tx,
		    u8 *rxbuf, const unsigned n_rx)
{
	struct spi_device *client = to_spi_device(xport->dev);
	struct rmi_spi_data *data = xport->data;
	struct rmi_device_platform_data *pdata = xport->dev->platform_data;
	int status;
	...
	if (pdata->spi_data.cs_assert) {
		...
		if (status) {
			dev_err(xport->dev, "Failed to assert CS, code %d.\n",
				status);
			/* nonzero means error */
			status = -1;
			goto error_exit;
	...
	if (pdata->spi_data.cs_assert) {
		...
		if (status) {
			dev_err(xport->dev, "Failed to deassert CS. code %d.\n",
				status);
			/* nonzero means error */
			status = -1;
			goto error_exit;
	...
	if (status == 0) {
		...
		if (n_rx)
			xport->info.rx_errs++;
		dev_err(xport->dev, "spi_sync failed with error code %d.",
		       status);
		goto error_exit;
	}
	...
	if (FF_DEBUG(data) && n_rx) {
		...
		if (bad_data) {
			xport->info.rx_errs++;
			dev_err(xport->dev, "BAD READ %lu out of %lu.\n",
				xport->info.rx_errs, xport->info.rx_count);
		}
	}
...
static int rmi_spi_v2_write_block(struct rmi_transport_device *xport, u16 addr,
				  const void *buf, const int len)
{
	...
	if (RMI_SPI_PAGE(addr) != data->page) {
		error = data->set_page(xport, RMI_SPI_PAGE(addr));
		if (error < 0)
			goto exit;
	}

	error = rmi_spi_xfer(xport, buf, len + 4, NULL, 0);
	if (error < 0)
		goto exit;
	error = len;
...
static int rmi_spi_v1_write_block(struct rmi_transport_device *xport, u16 addr,
				  const void *buf, const int len)
{
	struct rmi_spi_data *data = xport->data;
	u8 txbuf[len + 2];
	...
	error = rmi_spi_xfer(xport, txbuf, len + 2, NULL, 0);
	if (error < 0)
		goto exit;
	error = len;
...
static int rmi_spi_v2_split_read_block(struct rmi_transport_device *xport, u16 addr,
				       void *buf, const int len)
{
	...
	error = rmi_spi_xfer(xport, txbuf, 4, NULL, 0);
	if (error < 0) {
	...
	error = rmi_spi_xfer(xport, txbuf, 2, rxbuf, len + 1);
	data->split_read_pending = false;
	if (error < 0)
		goto exit;
...
static int rmi_spi_v2_read_block(struct rmi_transport_device *xport, u16 addr,
				 void *buf, const int len)
{
	struct rmi_spi_data *data = xport->data;
	u8 txbuf[4];
	...
	error = rmi_spi_xfer(xport, txbuf, 4, (u8 *) buf, len);
	if (error < 0)
		goto exit;
	error = len;
...
static int rmi_spi_v1_read_block(struct rmi_transport_device *xport, u16 addr,
				 void *buf, const int len)
{
	struct rmi_spi_data *data = xport->data;
	u8 txbuf[2];
	...
	error = rmi_spi_xfer(xport, txbuf, 2, (u8 *) buf, len);
	if (error < 0)
		goto exit;
	error = len;
...
static int rmi_spi_v1_set_page(struct rmi_transport_device *xport, u8 page)
{
	struct rmi_spi_data *data = xport->data;
	...
	error = rmi_spi_xfer(xport, txbuf, sizeof(txbuf), NULL, 0);
	if (error < 0) {
		dev_err(xport->dev, "Failed to set page select, code: %d.\n",
			error);
		return error;
	}
...
static int rmi_spi_v2_set_page(struct rmi_transport_device *xport, u8 page)
{
	struct rmi_spi_data *data = xport->data;
...
static int rmi_spi_check_device(struct rmi_transport_device *rmi_transport)
{
	u8 buf[6];
	int error;
	int i;
	 ...
	 */
	error = rmi_spi_v1_read_block(rmi_transport, PDT_START_SCAN_LOCATION,
				      buf, sizeof(buf));
	if (error < 0) {
		dev_err(rmi_transport->dev, "dummy read failed with %d.\n", error);
		return error;
	}
	 ...
	 */
	error = rmi_spi_v1_set_page(rmi_transport, 0x00);
	if (error < 0)
		return error;
	...
	error = rmi_spi_v1_read_block(rmi_transport, PDT_START_SCAN_LOCATION,
				      buf, sizeof(buf));
	if (error < 0) {
		dev_err(rmi_transport->dev, "probe read failed with %d.\n", error);
		return error;
	}
	...
	dev_err(rmi_transport->dev, "probe read returned invalid block.\n");
	return -ENODEV;
}
...
static int rmi_spi_probe(struct spi_device *spi)
{
	...
	rmi_transport = devm_kzalloc(&spi->dev, sizeof(struct rmi_transport_device),
			GFP_KERNEL);
	if (!rmi_transport)
		return -ENOMEM;
	...
	rmi_transport->dev = &spi->dev;

	rmi_transport->write_block = rmi_spi_v1_write_block;
	rmi_transport->read_block = rmi_spi_v1_read_block;
	rmi_transport->hard_irq = rmi_spi_hard_irq;
	data->set_page = rmi_spi_v1_set_page;

	rmi_transport->info.proto = spi_v1_proto_name;

	mutex_init(&data->page_mutex);

	dev_set_drvdata(&spi->dev, rmi_transport);
	...
	retval = rmi_spi_check_device(rmi_transport);
	if (retval < 0)
		goto err_gpio;

	/* check if this is an SPI v2 device */
	retval = rmi_spi_v1_read_block(rmi_transport, RMI_PROTOCOL_VERSION_ADDRESS,
				      buf, 2);
	if (retval < 0) {
	...
	if (buf[0] == 1) {
		/* SPIv2 */
		rmi_transport->write_block	= rmi_spi_v2_write_block;
		data->set_page		= rmi_spi_v2_set_page;

		rmi_transport->info.proto = spi_v2_proto_name;

		if (pdata->attn_gpio > 0) {
			init_completion(&data->irq_comp);
			rmi_transport->read_block = rmi_spi_v2_split_read_block;
		} else {
			dev_warn(&spi->dev, "WARNING: SPI V2 detected, but no attention GPIO was specified. This is unlikely to work well.\n");
			rmi_transport->read_block = rmi_spi_v2_read_block;
		}
	} else if (buf[0] != 0) {
