...
static int f11_allocate_control_regs(struct rmi_function *fn,
	...
	if (!ctrl->ctrl0_9)
		return -ENOMEM;
	if (sensor_query->query7_nonzero) {
	...
	if (sensor_query->query8_nonzero) {
	...
	if (f11->has_query9 && sensor_query->has_pen) {
	...
	if (f11->has_query9 && sensor_query->has_proximity) {
	...
	if (f11->has_query9 && (sensor_query->has_palm_det_sensitivity ||
